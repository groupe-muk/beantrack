...
{
    protected SupplierValidationService $validationService;

    public function __construct(SupplierValidationService $validationService)
    {
        $this->validationService = $validationService;
    }

    /**
     * Show the supplier onboarding page
     */
    ...
    /**
     * Store a new supplier application
     */
    public function store(Request $request): JsonResponse
    {
        try {
            // Authorization check - anyone can apply (public endpoint)
            $this->authorize('create', SupplierApplication::class);
            ...
            $validator = Validator::make($request->all(), [
                ...
                'email' => 'required|email|unique:supplier_applications,email|unique:users,email',
            ...
            Log::info('Supplier application submitted successfully', [
            ...
            Log::error('Unexpected error in supplier application submission', [
    ...
    {
        $this->authorize('viewAny', SupplierApplication::class);

        $query = SupplierApplication::query()->latest();
        ...
        return view('admin.supplier-applications.index', compact('applications'));
    }
    ...
    public function show(SupplierApplication $application)
    {
        try {
            ...
            \Log::error('Error in SupplierApplicationController@show: ' . $e->getMessage());
            if (request()->wantsJson() || request()->ajax()) {
                return response()->json([
                    'success' => false,
     ...
     */
    public function approve(SupplierApplication $application): JsonResponse
    {
     ...
     */
    public function reject(Request $request, SupplierApplication $application): JsonResponse
    {
        $this->authorize('update', $application);
     ...
     */
    public function downloadDocument(SupplierApplication $application, string $type)
    {
        $this->authorize('view', $application);
     ...
     */
    public function retryValidation(SupplierApplication $application): JsonResponse
    {
        $this->authorize('update', $application);
     ...
     */
    public function scheduleVisit(Request $request, SupplierApplication $application): JsonResponse
    {
        $this->authorize('update', $application);
        ...
        try {
            ...
            Log::info('Visit scheduled for supplier application', [
                'application_id' => $application->id,
            ...
            Log::error('Failed to schedule visit for supplier', [
                'application_id' => $application->id,
                'error' => $e->getMessage()
            ]);
