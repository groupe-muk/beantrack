...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <!-- Edit staff modals -->
        @foreach ($supplycenter->workers as $staff)
        <dialog id="edit-staff-{{ $staff->id }}" class="p-6 rounded bg-white shadow-lg max-w-md">
            <form method="POST" action="{{ route('worker.update', $staff) }}">
                @csrf
                @method('PATCH')
                <h3 class="text-lg font-bold mb-4">Edit Staff Member</h3>
                <input name="name" value="{{ $staff->name }}" class="border p-2 w-full mb-2" placeholder="Name">
                <input name="role" value="{{ $staff->role }}" class="border p-2 w-full mb-2" placeholder="Role">
                <input name="shift" value="{{ $staff->shift }}" class="border p-2 w-full mb-4" placeholder="Shift">
                <div class="flex justify-between">
                    <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded">Save</button>
                    <button type="button" onclick="this.closest('dialog').close()" class="bg-gray-300 px-4 py-2 rounded">Cancel</button>
                </div>
